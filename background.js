...
chrome.storage.sync.set({ runGLS: false });
...
chrome.browserAction.onClicked.addListener(function(activeTab){
    var newURL = "https://eportal.skku.edu";
    chrome.tabs.create({ url: newURL });
});

chrome.tabs.onUpdated.addListener(function (tabId, changeInfo, tab) {
    if (-1 < tab.url.indexOf("eportal.skku.edu/wps/myportal")) {
    ...
    if (-1 < tab.url.indexOf("admin.skku.edu/co/COCOUsrLoginAction.do")) {
        if (changeInfo.status == 'complete') {
            chrome.tabs.executeScript(tabId, {
                file: "loginAction.js",
                runAt: "document_idle"
...
function do_property(action, action_sub) {
    ...
    if (action == 'setproperty') {
        ...
        if (action_sub == 'basic')
            ...
            // 대소문자 구분 잘해서 사용!!!
            objMP.value = {
                "StartXML": strStartXML, "Width": "1024", "Height": "768",
